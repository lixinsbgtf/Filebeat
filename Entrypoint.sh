...
function updateIndexName(){
	...
	sed -i "s/sampleIndex/$1/g" /etc/*.yml
...
function updateTargetELK(){
	...
	sed -i "s#targetElkHost#$1#g" /etc/*.yml
	sed -i "s#ELASTIC_USERNAME#${ELASTIC_USERNAME}#g" /etc/*.yml
	sed -i "s#ELASTIC_PASSWORD#${ELASTIC_PASSWORD}#g" /etc/*.yml
